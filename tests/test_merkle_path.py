...
# @pytest.mark.asyncio
# async def test_verify_using_chain_tracker(tracker):
#    path = MerklePath(BRC74JSON['blockHeight'], BRC74JSON['path'])
#    result = await path.verify(BRC74TXID1, tracker)
...
def test_combine_paths():
    ...
    pathA = MerklePath(pathAJSON["blockHeight"], pathAJSON["path"])
    pathB = MerklePath(pathBJSON["blockHeight"], pathBJSON["path"])

    assert pathA.compute_root(BRC74TXID2) == BRC74Root
    with pytest.raises(ValueError):
        pathA.compute_root(BRC74TXID3)

    with pytest.raises(ValueError):
        pathB.compute_root(BRC74TXID2)
