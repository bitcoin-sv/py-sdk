...
class TestARCBroadcast(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_broadcast_success(self):
        mock_response = HttpResponse(ok=True, status_code=200, json_data={
            "txid": "8e60c4143879918ed03b8fc67b5ac33b8187daa3b46022ee2a9e1eb67e2e46ec", "txStatus": "success",
            "extraInfo": "extra"})
    ...
    async def test_broadcast_failure(self):
        mock_response = HttpResponse(ok=False, status_code=400,
                                     json_data={"status": "ERR_BAD_REQUEST", "detail": "Invalid transaction"})
        mock_http_client = AsyncMock(HttpClient)
        mock_http_client.fetch = AsyncMock(return_value=mock_response)
