...
class Writer:
    ...
    def write_reverse(self, buf: bytes) -> 'Writer':
        self.data.write(buf[::-1])
        return self

    def write_uint8(self, n: int) -> 'Writer':
        self.data.write(struct.pack('B', n))
        return self
    ...
    def write_uint16_be(self, n: int) -> 'Writer':
        self.data.write(struct.pack('>H', n))
        return self
    ...
    def write_uint32_be(self, n: int) -> 'Writer':
        self.data.write(struct.pack('>I', n))
        return self
    ...
    def write_var_int_num(self, n: int) -> 'Writer':
        self.data.write(self.var_int_num(n))
        return self
    ...
    def var_int_num(n: int) -> bytes:
        # TODO: Duplicate implementation (unsiged_to_varint) ?
        if n < 253:
...
class Reader:
    ...
    def read_var_int(self) -> bytes:
        first_byte = self.read(1)
        if first_byte[0] == 0xfd:
            return first_byte + self.read(2)
        elif first_byte[0] == 0xfe:
            return first_byte + self.read(4)
        elif first_byte[0] == 0xff:
            return first_byte + self.read(8)
        else:
            return first_byte